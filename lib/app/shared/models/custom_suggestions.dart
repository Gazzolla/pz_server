...
enum TextChangedReason {
  /// Whether the text in an [CustomAutoSuggestBox] was changed by user input
  userInput,

  /// Whether the text in an [CustomAutoSuggestBox] was changed because the user
...
/// An CustomAutoSuggestBox provides a list of suggestions for a user to select from
/// as they type.
///
/// ![CustomAutoSuggestBox Preview](https://docs.microsoft.com/en-us/windows/apps/design/controls/images/controls-autosuggest-expanded-01.png)
///
/// See also:
///
...
class CustomAutoSuggestBox extends StatefulWidget {
  /// Creates a fluent-styled auto suggest box.
  const CustomAutoSuggestBox({
    Key? key,
    required this.items,
    this.controller,
    this.onChanged,
  ...
  @override
  _CustomAutoSuggestBoxState createState() => _CustomAutoSuggestBoxState();
...
class _CustomAutoSuggestBoxState<T> extends State<CustomAutoSuggestBox> {
        ...
        child: CompositedTransformFollower(
          ...
          child: SizedBox(
            ...
            child: FluentTheme(
              ...
              child: _CustomAutoSuggestBoxOverlay(
                node: overlayNode,
...
class _CustomAutoSuggestBoxOverlay extends StatelessWidget {
  const _CustomAutoSuggestBoxOverlay({
    Key? key,
    required this.items,
      ...
      child: Container(
        ...
        child: ValueListenableBuilder<TextEditingValue>(
          ...
          builder: (context, value, _) {
            final items = CustomAutoSuggestBox.defaultItemSorter(
              value.text,
              this.items,
            );
            late Widget result;
            if (items.isEmpty) {
              result = Padding(
                ...
                child: _CustomAutoSuggestBoxOverlayTile(text: localizations.noResultsFoundLabel),
              );
            } else {
              result = ListView(
                ...
                children: List.generate(items.length, (index) {
                  ...
                  return _CustomAutoSuggestBoxOverlayTile(
                    text: '$item',
...
class _CustomAutoSuggestBoxOverlayTile extends StatefulWidget {
  const _CustomAutoSuggestBoxOverlayTile({
    Key? key,
    required this.text,
    this.onSelected,
  }) : super(key: key);
  ...
  @override
  __CustomAutoSuggestBoxOverlayTileState createState() => __CustomAutoSuggestBoxOverlayTileState();
}

class __CustomAutoSuggestBoxOverlayTileState extends State<_CustomAutoSuggestBoxOverlayTile> with SingleTickerProviderStateMixin {
